...
function updateTrackInfo() {
  ...
  if (artworkEl) {
    const parts = [];
    if (visName) parts.push(`visual: ${visName}`);
    if (filtName && filtName !== 'none') parts.push(`fx: ${filtName}`);
    artworkEl.textContent = parts.join('  ');
  }
...
function showDataToPlaylist(showData) {
  const epoch = showData.generated_at
    ? new Date(showData.generated_at).getTime()
    : Date.now();
  return {
    epoch,
    ...
    tracks: (showData.tracks || []).map((t, i) => ({
      ...
      artist: t.artist || showData.dj_name,
      caption: t.caption || '',
      bpm: t.bpm,
      key: t.key,
      voiceoverFile: t.voiceover_file,
    })),
  };
}
