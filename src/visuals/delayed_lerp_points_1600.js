...
visuals.register('delayed_lerp_points_1600', (() => {
  let myP5 = null;
  ...
  function start() {
    ...
    myP5 = new p5((p) => {
      ...
      function drawTrail() {
        ctx.rotate(angle);
        const sz = Math.abs(Math.sin(p.radians(p.frameCount / 10)) * circ_start_sz);
        ctx.push();
        ...
        ctx.fill(p.random(200), p.random(223), p.random(200), 1.5);
  ...
  return { name: 'Delayed Lerp Points 1600', start, stop };
})());
