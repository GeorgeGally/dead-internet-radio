...
visuals.register('led_grid', (() => {
  ...
  function start() {
    ...
    myP5 = new p5((p) => {
      ...
      function moveBall(b) {
        ...
        if (spectrum.length > 0) {
          ...
          if (mode === 0) {
            ...
            s = mapSound(new_me, balls.length * 2, 10, 100) * gain;
          } else if (mode === 1) {
            s = mapSound(b.me, balls.length * 1, 10, 100) * gain;
          } else if (mode === 5) {
            const new_me = b.me % 150;
            s = mapSound(new_me, 150, 10, 100) * gain;
          } else if (mode === 6) {
            s = mapSound(b.me, balls.length, 10, 100) * gain;
          } else if (mode === 7) {
            const new_me = b.me % 50;
            s = mapSound(new_me, 100, 0, 200) * gain;
          } else {
            s = mapSound(b.me, balls.length, 10, 100) * gain;
          }
        }
      ...
      function led() {
        // Map _cutoffVal (5–255) → sz (2–32): low cutoff = fine grid, high = coarse
        const cutoff = window._cutoffVal != null ? window._cutoffVal : 130;
        sz = Math.max(2, Math.round(2 + (cutoff / 255) * 30));
      ...
      p.draw = () => {
        ...
        if (p.frameCount % 20 === 0) {
          if (chance(300)) mode = randomInt(0, 8);
          if (chance(400)) speed *= -1;
          if (chance(400)) pix = !pix;
          if (chance(80)) reset();
        }
        ...
        if (pix && p.frameCount % 4 === 0 && chance(8)) randomPixelate();
        led();
