'use strict';

visuals.register('cubes_token_sha', (() => {
  let myP5 = null;
  let blockCanvas = null;

  function start() {
    blockCanvas = document.getElementById('block-canvas');
    const gifBg = document.getElementById('gif-bg');
    const overlay = document.getElementById('overlay-canvas');
    if (blockCanvas) blockCanvas.style.display = 'none';
    if (gifBg) gifBg.style.display = 'none';
    if (overlay) overlay.style.display = 'none';

    myP5 = new p5((p) => {
      const s = 60;
      const dims = 8;
      const DEFAULT_DIST = 1100;
      const SPRING_K = 0.035;
      const DAMPING = 0.88;
      let hash;
      let r;
      let c = [];
      let rotX = -0.4;
      let rotY = 0.5;
      let zoomOffset = 0;
      let zoomVel = 0;
      let isDragging = false;
      let prevX = 0;
      let prevY = 0;

      function sha512(str) {
        return crypto.subtle
          .digest('SHA-512', new TextEncoder('utf-8').encode(str))
          .then((buf) => {
            return Array.prototype.map
              .call(new Uint8Array(buf), (x) => ('00' + x.toString(16)).slice(-2))
              .join('');
          });
      }

      function addCubes(sha) {
        let d = 0;
        for (let x = 0; x < dims; x++) {
          for (let y = 0; y < dims; y++) {
            for (let z = 0; z < dims; z++) {
              let b = 0;
              if (sha.substring(d, d + 1).charCodeAt(0) > 60) {
                b = 1;
              }
              const a = addCube(
                x * s - 4.5 * s + s / 2,
                y * s - 4.5 * s + s / 2,
                z * s - 4.5 * s + s / 2,
                b
              );
              c.push(a);
              d++;
            }
          }
        }
      }

      function addCube(x, y, z, b) {
        const col = p.color(r[0] % 256, p.random(255), p.random(255));
        if (b === 0) {
          return { x, y, z, c: col };
        }
        const ball = {
          x: 0, y: 0, z: 0,
          sx: p.random(-2, 2),
          sy: p.random(-2, 2),
          sz: p.random(-2, 2)
        };
        return { x, y, z, ball, c: col, b };
      }

      function moveBall(b) {
        b.x += b.sx;
        b.y += b.sy;
        b.z += b.sz;
        const half = s / 2;
        if (b.x >= half || b.x <= -half) b.sx *= -1;
        if (b.y >= half || b.y <= -half) b.sy *= -1;
        if (b.z >= half || b.z <= -half) b.sz *= -1;
      }

      function drawBall(cube) {
        const b = cube.ball;
        p.fill(cube.c);
        p.noStroke();
        p.translate(b.x, b.y, b.z);
        p.sphere(3);
      }

      function drawCube(cube) {
        if (cube.b === 1) {
          p.stroke(60);
          p.noFill();
        } else {
          p.fill(28, 28, 32);
          p.noStroke();
        }
        p.box(s);
      }

      p.setup = () => {
        visuals.setupP5Canvas(p, { renderer: p.WEBGL, preserveDrawingBuffer: true, pixelDensity: 1 });

        const tokenData = {
          hash: '0x11ac16678959949c12d5410212301960fc496813cbc3495bf77aeed738579738',
          tokenId: '123000456',
        };

        hash = tokenData.hash.substr(2, 66);
        r = Uint32Array.from([0, 0, 0, 0].map((_, i) => parseInt(hash.substr(i * 8 + 2, 8), 16)));

        const h1 = hash.substr(0, 16);
        const h2 = hash.substr(16, 32);
        const h3 = hash.substr(32, 48);
        const h4 = hash.substr(48, 64);

        sha512(h1).then((x1) => {
          let sha = x1;
          sha512(h2).then((x) => {
            sha += x;
            sha512(h3).then((x) => {
              sha += x;
              sha512(h4).then((x) => {
                sha += x;
                addCubes(sha);
              });
            });
          });
        });
      };

      p.draw = () => {
        p.background(233, 230, 223);

        const springForce = -SPRING_K * zoomOffset;
        zoomVel = (zoomVel + springForce) * DAMPING;
        zoomOffset += zoomVel;

        if (!isDragging) {
          rotY += 0.003;
        }

        const dist = DEFAULT_DIST + zoomOffset;
        p.camera(0, 0, dist, 0, 0, 0, 0, 1, 0);
        p.rotateX(rotX);
        p.rotateY(rotY);

        for (let i = 0; i < c.length; i++) {
          const pCube = c[i];
          p.push();
          p.translate(pCube.x, pCube.y, pCube.z);
          drawCube(pCube);
          if (pCube.ball) {
            moveBall(pCube.ball);
            drawBall(pCube);
          }
          p.pop();
        }
        canvasFilters.applyToP5(p);
      };

      p.mouseWheel = (event) => {
        zoomOffset += event.delta * 0.5;
        zoomOffset = Math.max(zoomOffset, -DEFAULT_DIST * 0.4);
        zoomOffset = Math.min(zoomOffset, 500);
        return false;
      };

      p.mousePressed = () => {
        if (p.mouseButton === p.LEFT) {
          isDragging = true;
          prevX = p.mouseX;
          prevY = p.mouseY;
        }
      };

      p.mouseReleased = () => {
        isDragging = false;
      };

      p.mouseDragged = () => {
        if (isDragging) {
          const dx = p.mouseX - prevX;
          const dy = p.mouseY - prevY;
          rotY += dx * 0.005;
          rotX += dy * 0.005;
          rotX = Math.max(-Math.PI / 2, Math.min(Math.PI / 2, rotX));
          prevX = p.mouseX;
          prevY = p.mouseY;
        }
      };

      p.touchStarted = () => {
        if (p.touches.length === 1) {
          isDragging = true;
          prevX = p.touches[0].x;
          prevY = p.touches[0].y;
        }
      };

      p.touchMoved = () => {
        if (isDragging && p.touches.length === 1) {
          const dx = p.touches[0].x - prevX;
          const dy = p.touches[0].y - prevY;
          rotY += dx * 0.005;
          rotX += dy * 0.005;
          rotX = Math.max(-Math.PI / 2, Math.min(Math.PI / 2, rotX));
          prevX = p.touches[0].x;
          prevY = p.touches[0].y;
        }
        return false;
      };

      p.touchEnded = () => {
        isDragging = false;
      };
    });
  }

  function stop() {
    visuals.stopP5(myP5);
    myP5 = null;
  }

  return { name: 'Cubes Token SHA', start, stop };
})());
